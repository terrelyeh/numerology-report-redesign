...
| 無價格錨定 | 沒有原價/現價對比，$9.90 看起來像是隨意定價 |
...
解讀的深度和個人化程度其實值得 $9.90，但使用者感受不到。需要新增 Action Insights（行動建議）、PDF 下載、視覺升級，讓付費體驗「明顯不同且值得」。
...
付費報告與免費報告在視覺和內容上缺乏明顯區隔。使用者付了 $9.90 後感覺「跟免費的差不多」，這是導致退款和負面口碑的主因。
...
- 原價 `$19.90` 以刪除線顯示（`line-through`）
- 現價 `$9.90` 以大字體（`text-5xl`）顯示
- 暗示節省了 $10（50% off），但不明確標示折扣比例（避免「打折貨」印象）
...
| 價格錨定 | 直接顯示 $9.90 | ~~$19.90~~ → $9.90 | 付款完成率 |
...
- [ ] **定價 A/B 測試**：測試不同價格點（$7.90 / $9.90 / $14.90）

---

## 九、商業模式與營收成長優化

目前的單次購買模式讓客戶終身價值（LTV）天花板很低。以下方向可在不大改架構的前提下，有效拉高每位使用者的營收貢獻。

### 9.1 現行模式分析

**現行定價**：$9.90 / 份（一個生日 = 一份報告）

| 機制 | 狀態 | 問題 |
|------|------|------|
| 註冊後可重複購買 | ✅ 支援 | 但網站無任何引導使用者購買第二份的機制 |
| 可購買他人的報告 | ✅ 支援 | 使用者不知道可以幫家人買，無 Gift 功能提示 |
| 訂閱制 / 回訪內容 | ❌ 無 | 報告靜態，買完看完就結束，無回訪理由 |
| 配對 / 關係報告 | ❌ 無 | 缺少自然的第二次購買動機 |

> **核心瓶頸**：功能上已經支援多份購買，但**沒有任何 UX 引導**讓使用者知道可以這樣做。能力有了，缺的是引導。

### 9.2 關係配對報告（最推薦方向）

配對報告是最有潛力的新產品線，原因：

1. **自然的購買動機**：使用者讀完自己的報告，最直覺的下一個念頭：「那我跟我另一半呢？」不需要刻意推銷
2. **全新的產品體驗**：不是「再買一份一樣的」。配對報告分析兩人數字的互補、衝突、溝通模式——是使用者從未見過的內容
3. **信任已建立**：使用者已經體驗過個人報告的準確度，對系統有信心。這時推配對報告，轉換門檻極低

#### 配對報告可分析的維度（範例）

| 維度 | 說明 |
|------|------|
| 溝通風格配對 | 兩人的 Ego Number 如何影響日常溝通？一個是 9（理想主義）配一個 3（表達型）會怎樣？ |
| 衝突觸發點 | 分析兩人 Frustration Connection 的交叉點，預測最可能的摩擦來源 |
| 互補優勢 | 一個人的 Isolated Number 可能正好是對方的強項，形成天然互補 |
| 關係時間線 | 兩人的 Peak Pyramids 疊加分析——哪些年份是關係的轉捩點？ |

#### 定價建議

- **獨立購買**：$9.90（與個人報告同價，降低決策門檻）
- **加購優惠**：買完個人報告後加購 $6.90（立即結帳折扣）
...
```
層級 1（免費）：Birth Chart + 部分解讀
    ↓
層級 2（$9.90 一次性）：完整個人報告 + 12 個月預測
    ↓
層級 3（+$6.90 加購）：配對報告
    ↓
層級 3b（+$9.90）：幫家人 / 朋友買額外報告
    ↓
層級 4（$4.99/月 訂閱，未來）：月度能量更新
```
...
- [ ] **報告完成頁加上「幫家人算一份」CTA**：使用者讀完報告情緒最高點，推「Want to understand someone in your life better?」→ 輸入對方生日
- [ ] **付款確認 Email 內加回購鉤子**：「Your numbers interact differently with the people around you. Try a Compatibility Reading.」

**P2 — 本月可做：**

- [ ] **多份折扣機制**：第二份起 $7.90（8 折），或 Family Bundle 3 份 $19.90（原價 $29.70）

**P3 — 需開發新功能：**

- [ ] **配對報告產品線**：新報告類型，輸入兩人生日 → 分析數字互補與衝突。獨立 $9.90 或加購 $6.90

**未來方向：**

- [ ] **月度訂閱制**：月度預測不一次全給，改為每月初寄送新能量信。$4.99/月 或 $29.90/年
...
Attributes 頁面（`/attributes`）是一個命理名詞百科頁，以左側目錄 + 右側文章的方式，將 11 個命理概念的通用定義全部放在同一頁上。

**架構問題：11 個主題擠在一頁**

目前所有主題都在 `/attributes` 一頁內。這帶來兩個問題：
1. **SEO 稀釋**：Google 把整頁當一篇文章索引，11 個關鍵字互相競爭，很難針對單一關鍵字（例如「what is life path number」）取得排名
2. **CTA 尷尬**：如果每個主題後面都加一個 CTA，一頁就有 11 個 CTA，使用者會覺得「這頁一直在叫我去算」，反而降低信任感

**建議：拆成 11 個獨立頁面**

```
現況：/attributes（一頁 11 個主題，約 5000 字混在一起）

建議：
  /attributes/birth-chart
  /attributes/identity-number
  /attributes/ego-number
  /attributes/superego-number
  /attributes/isolated-number
  /attributes/connection-patterns
  /attributes/life-path-number
  /attributes/attitude-number
  /attributes/day-of-birth-number
  /attributes/key-life-ages
  /attributes/personal-year-number
```

| | 一頁（現況） | 11 頁（建議） |
|---|---|---|
| SEO | 只索引一個 URL，關鍵字互相稀釋 | 每頁針對一個關鍵字，排名機會 ×11 |
| CTA | 放 11 個太多，放 1 個太少 | 每頁底部一個 CTA，非常自然 |
| 內容深度 | 擠在一起，每個偏短 | 各自 800-1200 字，有深度 |
| 分享性 | 只能分享整頁 | 可分享單篇到社群、論壇 |

> **如果短期拆頁成本太高**，折衷方案是維持一頁，但只在第 5-6 個主題之後和頁面最底部各放一個 CTA（共 2 個），避免過度推銷感。
...
#### B. 每篇文章底部加導流 CTA（拆頁後適用）

拆成獨立頁面後，每篇文章結尾加一個 CTA 區塊（一頁只有一個，不會突兀）：
